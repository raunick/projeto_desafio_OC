        ...
        "series": [
            {
                ...
                "title": {"offsetCenter": [0, "70%"], "textStyle": {
                    "color": ""}},
                "detail": {
                    "fontSize": 30,
                    "fontWeight": "bolder",
...
def painelPA():
    ...
    if filas == "🏥 Todos":
        ...
        with col1:
            ...
            ler_csv(f"{pasta}TRIAGEM.csv", pd)
            st.write("💻 Tabela CADASTRO:")
            ler_csv(f"{pasta}CADASTRO.csv", pd)
            st.write("👨‍⚕️ Tabela MEDICO:")
            ler_csv(f"{pasta}MEDICO.csv", pd)
        with col2:
        ...
        df = ler_csv_all(f"{pasta}filas.csv", pd)
        ...
        with open(f"{pasta}arquivo.txt", "r", encoding="utf-8") as f:
            codigo = f.read()
